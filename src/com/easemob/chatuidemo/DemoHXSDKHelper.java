...
public class DemoHXSDKHelper extends HXSDKHelper{
    ...
    private EMValueCallBack<User> userCallBack;
    public void asyncGetUserInfo(String username,
            EMValueCallBack<User> emValueCallBack) {
        File cache = new File(Environment.getExternalStorageDirectory(),"cache");
        if(!cache.exists()){
            cache.mkdirs();
        }   
        this.userCallBack = emValueCallBack;
    ...
    public class GetUserInfoTask extends AsyncTask<String, Void, User> {
        ...
        protected User doInBackground(String... arg0) {
            ...
            if (file.exists()) {
                ...
                try {  
                    ...
                    params.put("user", arg0[0]);
                    ConnServer server = new ConnServer();
                    String path = "http://"+getAppContext().getResources().getString(R.string.server)+"/api.php"; 
                
                    ListData = server.sendPOSTRequire(path,params,"UTF-8",new String[]{});
                    if(ListData.size() > 0){
                        ListData = server.Json2List(ListData.get(0).get("data").toString());
                        // 从网络上获取图片  
                        URL url = new URL(ListData.get(0).get("avatar").toString());  
        ...
        protected void onPostExecute(User user) {
            ...
            //Log.e("user1","name="+user.getUsername()+";avatar="+user.getAvatar());
    ...
    public void uploadUserAvatar(InputStream data, final EMValueCallBack<String> emValueCallBack) {
        ...
        // 上传文件  
        client.post("http://"+getAppContext().getResources().getString(R.string.server)+"/api.php", params, new AsyncHttpResponseHandler() {  
            ...
            public void onSuccess(int statusCode, Header[] headers,  
                ...
                try {
                    ...
                    if(Boolean.valueOf(Data.get(0).get("status").toString())){
                        emValueCallBack.onSuccess(Data.get(0).get("msg").toString());
                    }else{
                        emValueCallBack.onError(1,Data.get(0).get("msg").toString());
                    }
                } catch (Exception e) {
                    // TODO Auto-generated catch block
                    e.printStackTrace();
                    emValueCallBack.onError(2, "fail");
                }
            }  
            ...
            public void onFailure(int statusCode, Header[] headers,  
                ...
                // 上传失败后要做到工作  
                emValueCallBack.onError(3, "fail");
            }  
