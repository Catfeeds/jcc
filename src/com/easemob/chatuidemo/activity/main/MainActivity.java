...
import com.easemob.chatuidemo.activity.MessageActivity;
...
public class MainActivity extends BaseActivity implements EMEventListener {
	...
	private SeikatsuFragment seikatsuFragment;
    public SchoolyardFragment schoolyardFragment;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		DemoApplication.getInstance().setHandler(uHandler);
		...
		// 显示所有人消息记录的fragment
		seikatsuFragment = new SeikatsuFragment();
		schoolyardFragment = new SchoolyardFragment();
		chatHistoryFragment = new ChatAllHistoryFragment();
		contactListFragment = new ContactlistFragment();
		settingFragment = new SettingsFragment();
		fragments = new Fragment[] { schoolyardFragment, seikatsuFragment };
		// 添加显示第一个fragment
		FragmentTransaction transaction = getSupportFragmentManager().beginTransaction().add(R.id.fragment_container, schoolyardFragment)
				.add(R.id.fragment_container, seikatsuFragment);

        switch (identity) {
        case 2:
            currentTabIndex = 1;
            mTabs[1].setSelected(true);
            transaction.hide(schoolyardFragment).show(seikatsuFragment);
            break;
        default:
            currentTabIndex = 0;
            mTabs[0].setSelected(true);
            transaction.hide(seikatsuFragment).show(schoolyardFragment);
            break;
        }
        transaction.commit();
            ...
            public void onSuccess(List<String> usernames) {
                ...
                ((DemoHXSDKHelper)HXSDKHelper.getInstance()).getUserProfileManager().asyncFetchContactInfosFromServer(usernames,new EMValueCallBack<List<User>>() {
					...
					public void onSuccess(List<User> uList) {
					    System.out.println("MainActivity-update:"+uList);
	...
	public void onEvent(EMNotifierEvent event) {
        ...
        case EventNewCMDMessage:
        {
            ...
            try {
                ...
                String subject = message.getStringAttribute("subject");
            } catch (EaseMobException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }
            EMLog.d(TAG, String.format("Main透传消息：action:%s,message:%s", action,message.toString()));
            ...
            HXSDKHelper.getInstance().getNotifier().onNewCMDMsg(message);
            refreshUI();
...
//				}
	...
	public void updateUnreadLabel() {
	    mInviteNum = getUnreadMsgCountTotal();
		mChatNum = getUnreadMsgCountTotal();
		if (mInviteNum > 0 || mChatNum > 0) {
//		    unreadAddressLable.setText(String.valueOf(count));
		...
		if(mInformNum > 0)
		    schoolyardFragment.unreadHomework.setVisibility(View.VISIBLE);
		else
		    schoolyardFragment.unreadHomework.setVisibility(View.GONE);
	}
	...
	public void updateUnreadAddressLable() {
		runOnUiThread(new Runnable() {
			public void run() {
			    mInviteNum = getUnreadAddressCountTotal();
			    mChatNum = getUnreadMsgCountTotal();
				if (mInviteNum > 0 || mChatNum > 0) {
//					unreadAddressLable.setText(String.valueOf(count));
					unreadAddressLable.setVisibility(View.VISIBLE);
...
//			contactListFragment.refresh();
	...
	private void showConflictDialog() {
		...
		if (!MainActivity.this.isFinishing()) {
			...
			try {
                ...
                isConflict = true;
				DemoApplication.getInstance().setConflict(isConflict);
			} catch (Exception e) {
	...
	private void showAccountRemovedDialog() {
		...
		if (!MainActivity.this.isFinishing()) {
			...
			try {
                ...
                DemoApplication.getInstance().setCurrentAccountRemoved(true);
    ...
    private List<Map<String, Object>> mUserData = new ArrayList<Map<String,Object>>();
    /**
     * 获取服务器用户信息
     */
	private class GetDataThread extends Thread { 
	        ...
	        public void run() {
	            ...
	            mUserData = server.GetData(path,params,edit,uHandler);
	        }
	    }
	
	private void setUserInfo(){
	    try {
	        DemoApplication.getInstance().setNick(mUserData.get(0).get("nick").toString());
	        DemoApplication.getInstance().setAvatar(mUserData.get(0).get("avatar").toString());
	        DemoApplication.getInstance().setIdentity(Integer.valueOf((mUserData.get(0).get("identity").toString())));
            DemoApplication.getInstance().setTutorial(Integer.valueOf((mUserData.get(0).get("tutorial").toString())));
        } catch (Exception e) {
            e.printStackTrace();
        }
	}
