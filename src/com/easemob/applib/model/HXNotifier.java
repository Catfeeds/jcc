...
public class HXNotifier {
    ...
    protected int notificationCMDNum = 0;
    ...
    public synchronized void onNewCMDMsg(EMMessage message) {
        if(EMChatManager.getInstance().isSlientMessage(message)){
            return;
        }
        ...
        if (!EasyUtils.isAppRunningForeground(appContext)) {
            EMLog.d(TAG, "app is running in backgroud");
            sendNotification2(message, false, true);
        } else {
            sendNotification2(message, true, true);
     ...
     * @param message
     */
    protected void sendNotification2(EMMessage message, boolean isForeground, boolean numIncrease) {
        String username = message.getFrom();
        try {
            String notifyText = "有新的通知";
            ...
            try {
                ...
                Log.e("", String.format("HXNotifier透传消息：action:%s,message:%s,attr:[id:%s],[subject:%s]", action,message.toString(), id, subject));
            } catch (EaseMobException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }
            if(action.equals("inform")){
                intent = new Intent(appContext, HomeworkInfoActivity.class);
                notifyText = "有新的作业信息通知";
            ...
            String summaryBody = notifyText.replaceFirst("新的", Integer.toString(notificationCMDNum)+"条");
            
            if (notificationInfoProvider != null) {
                // lastest text
                String customSummaryBody = notificationInfoProvider.getLatestText(message, fromUsersNum,notificationCMDNum);
